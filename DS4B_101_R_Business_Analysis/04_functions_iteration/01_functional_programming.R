...
rolling_avg_3_tbl <- bike_orderlines_tbl %>%
    ...
    ungroup() %>%
    mutate(category_2 = as_factor(category_2) %>% fct_reorder(month_end, total_price))
    view()
...
class_detect <- function(x) {
    if (is.numeric(x)) {
        ...
        stop("In class_detect(): Value is character! Should be numeric, but can be accepted", call. = FALSE)
        print(x)
    } else {
...
detect_outliers <- function(x) {
    ...
    limits_tbl <- data_tbl %>%
        summarise(
            ...
            limit_low = quantile_lo - 1.5*iqr,
            limit_hi = quantile_hi + 1.5 * iqr
        )
    
    output_tbl <- data_tbl %>%
        mutate(outlier = case_when(
            data < limits_tbl$limit_low ~ TRUE,
...
fs::dir_create("Desktop/DS4B_101_R_Business_Analysis/00_scripts/")

path <- "Desktop/DS4B_101_R_Business_Analysis/00_scripts/separate_bikes_and_outlier_detection.R"
fs::file_create(path)
...
# separate_bike_modles(): A tidy function to separate the model column into engineered features
...
c("separate_bike_model", "detect_outliers") %>%
    dump(file = "Desktop/DS4B_101_R_Business_Analysis/00_scripts/separate_bikes_and_outlier_detection.R",
         append = TRUE)
...
source("Desktop/DS4B_101_R_Business_Analysis/00_scripts/separate_bikes_and_outlier_detection.R")
