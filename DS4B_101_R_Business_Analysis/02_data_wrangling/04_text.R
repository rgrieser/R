...
bikes_tbl <- readxl::read_excel("Desktop/DS4B_101_R_Business_Analysis/00_data/bike_sales/data_raw/bikes.xlsx")
...
"  text with space  " %>% str_trim(side = "right")
...
# removing all numbers and replace with an "x"
...
(value / 1e6) %>%
    ...
    (1e6 / 1) %>% scales::number(prefix = "$", suffix = "M")
